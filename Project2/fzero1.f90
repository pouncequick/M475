	PROGRAM Main Rootfinder
	Real Func, b, c, x, re, ae, iflag
	Func = x**3 - 1.2*x**2 - 8.55*x + 12.15
	b = -4
	c = 1
	ae = 0
	re = 0
	iflag = 0
	CALL fzero(Func,b,c,re,ae,iflag)
	END	
...
!
!        R     - A (better) guess of a zero of Func which could help in  
!**** Removed ***
...
!Make sure last call is for smallest function value
            fc = Func( c )
            ELSE
            fb = Func( b )
            END IF
...
!------ Get new iterate
        ...
        IF( p < ABS(q)*tol )THEN 
!If too small, increment by tolerance
            b = b + SIGN(tol,cmb)
        ELSE IF( p < cmb*q )THEN 
!Use secant rule if root is between b and (c+b)/2
            b = b + p/q
        ELSE                     
!Othewise, use bisection
            b = 0.5*(c+b)
        END IF
